    ...
    if st.button("🚀 GENERATE INTERVIEW QUESTIONS"):
        if int_resume and int_jd:
            with st.spinner("Building personalized question set from your CV & JD..."):
                ...
                st.session_state["int_resume_text"] = txt
                st.session_state["int_jd_text"] = int_jd
    ...
    if "interview_qs" in st.session_state:
        ...
        for i, q in enumerate(qs):
            ...
            <div class="q-card">
                ...
                # Answer Evaluation for each question
                with st.expander(f"🗣️ Practice Answer (Question {i+1})"):
                    mode = st.radio("Input Method", ["Text Input", "Live Record", "Upload File"], key=f"mode_{i}")
                    ...
                    if mode == "Text Input":
                        user_text_input = st.text_area("Type your answer here...", key=f"text_ans_{i}", height=120)
                        if st.button(f"🧠 Evaluate Answer {i+1}", key=f"eval_btn_txt_{i}"):
                            if user_text_input.strip():
                                submit_eval = True
                            else:
                                st.warning("Please type an answer first.")
                    elif mode == "Live Record":
                        ...
                        if audio_bytes and st.button(f"🧠 Evaluate Answer {i+1}", key=f"eval_btn_live_{i}"):
                            submit_eval = True
                    else:
                        ...
                        if audio_bytes and st.button(f"🧠 Evaluate Answer {i+1}", key=f"eval_btn_up_{i}"):
                            submit_eval = True

                    if submit_eval:
                        final_text_to_eval = user_text_input
                        
                        if mode in ["Live Record", "Upload File"] and audio_bytes:
                            with st.spinner("Analyzing Speech..."):
                                eval_res = evaluate_audio_answer(audio_bytes, audio_format)
                                if eval_res["success"]:
                                    final_text_to_eval = eval_res['transcription']
                                    st.markdown(f"**Transcription:** *\"{final_text_to_eval}\"*")
                                    ...
                                    st.info(f"**Vocal Feedback:** {eval_res['feedback']}")
                                else:
                                    st.error(eval_res["error"])
                                    final_text_to_eval = ""
                        
                        if final_text_to_eval.strip():
                            with st.spinner("Evaluating Content with AI..."):
                                c_jd = st.session_state.get("int_jd_text", "")
                                c_cv = st.session_state.get("int_resume_text", "")
                                answer_eval = st.session_state.analyzer.evaluate_interview_answer(
                                    question=q['question'],
                                    sample_answer=q.get('sample_answer', q['talking_point']),
                                    user_answer=final_text_to_eval,
                                    jd_text=c_jd,
                                    resume_text=c_cv
                                )
                                ...
                                color = "#4CFF91" if score >= 80 else ("#FFA500" if score >= 60 else "#FF6B6B")
                                
                                st.markdown(f"<h3 style='color:{color};'>Score: {score}/100</h3>", unsafe_allow_html=True)
                                st.write(f"**Feedback:** {answer_eval.get('feedback', '')}")
                                
                                if answer_eval.get("strengths"):
                                    st.markdown("**✨ Strengths:**")
                                    for s in answer_eval["strengths"]:
                                        st.markdown(f"- {s}")
                                
                                if answer_eval.get("areas_for_improvement"):
                                    st.markdown("**🔧 Areas for Improvement:**")
                                    for a in answer_eval["areas_for_improvement"]:
                                        st.markdown(f"- {a}")
